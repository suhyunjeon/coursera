...
object Huffman {
   ...
   */
    ...
    def times(chars: List[Char]): List[(Char, Int)] = {
      def loop(charList: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = charList match {
        case Nil => acc
        case x :: xs => loop(xs.filter(_ != x), (x, charList.count(_ == x)) :: acc)
      }
      loop(chars, List())
    }.sorted
...
//    Fork(left, right, chars(left) ::: chars(right), weight(left) + weight(right))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: rest => (makeCodeTree(left, right) :: rest).sortBy(weight(_))
    case _ => trees
    }
   ...
   */
    /*def decode(tree: CodeTree, bits: List[Bit]): List[Char] = tree match {
      ...
      case Fork(left, right, _, _) => if (bits.isEmpty) decode(left, bits.tail) else decode(right, bits.tail)
    }*/
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def loop(tree:  CodeTree)(c: Char): List[Bit] = tree match {
      case Leaf(_, _) => List()
      case Fork(left, _, _, _) if chars(left).contains(c) => 0 :: loop(left)(c)
      case Fork(_, right, _, _) => 1 :: loop(right)(c)
    }
