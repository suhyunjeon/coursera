...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toList.groupBy(_.toLower).map({case (a, b) => (a, b.length)}).toList.sortWith(_._1 < _._1)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def acc(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(Nil)
      else for {
        combination <- combinations( occurrences )
        word <- dictionaryByOccurrences getOrElse (combination, Nil)
        sentence <- acc( subtract(occurrences,wordOccurrences(word)) )
